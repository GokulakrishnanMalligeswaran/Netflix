...
const user_auth=async (event:React.MouseEvent<HTMLButtonElement>)=>{
  ...
  return (
    loading?<div className="w-full h-screen flex items-center justify-center sm:py-5 px-4">
      <img src={netflix_spinner} alt="" className="w-14"/>
    </div>:
    <div
      className="flex flex-col items-center w-full h-screen py-[100px] sm:[min-w-[640px] h-screen]"
      style={{ backgroundImage: `url(${background_image})` }}
    >
      <div className="bg-black bg-opacity-80 px-10 py-10 rounded-md sm:min-w-[600px]">
